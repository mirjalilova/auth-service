...
import (
	...
	"github.com/mirjalilova/auth-service/api/token"
)
...
func (r *AuthRepo) Register(req *pb.RegisterReq) (*pb.Void, error) {
	...
	var id string
	query := `INSERT INTO users (username, email, password, full_name, date_of_birth, gender, phone_number, country) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`
	err = tr.QueryRow(query, req.Username, req.Email, req.Password, req.FullName, req.DateOfBirth, req.Gender, req.PhoneNumber, req.Country).Scan(&id)
	if err != nil {
		tr.Rollback()
		return nil, err
	}
