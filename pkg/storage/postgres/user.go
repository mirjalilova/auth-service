...
func (r *UserRepo) GetProfile(req *pb.GetById) (*pb.UserRes, error) {
	...
	var date string
	query := `SELECT id, username, email, full_name, date_of_birth, gender, phone_number, country, role FROM users WHERE id = $1`
...
func (r *UserRepo) EditProfile(req *pb.UserRes) (*pb.UserRes, error) {
	...
	if req.Country != "" && req.Country != "string" {
		arg = append(arg, req.Country)
		conditions = append(conditions, fmt.Sprintf("country = $%d", len(arg)))
	}
